...
const WithdrawalTx = ({ title, date, amount, token, status }: WithdrawalTxProps) => {
    return (
        <main className="w-full flex justify-between items-center cursor-pointer hover:bg-[#F9FAFB] transition-all px-2 py-2 rounded-[8px]">
            <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col gap-1">
                    <h4 className="text-strimzPrimary text-sm font-[500] font-poppins">{title}</h4>
                    <p className="text-xs font-[400] font-poppins text-[#8E8C9C]">{date}</p>
                </div>
            </div>
            <div className="flex flex-col items-end gap-1">
                <h4 className="text-strimzPrimary text-sm font-[500] font-poppins">${amount}</h4>
